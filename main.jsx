...
const DawnDeltaTool = () => {   // closing brace at end of the file, pretty much
...
const getLocalesForStartDate = useCallback((start) => {
  return dstLocales.filter(loc => loc.start === start).map(loc => loc.name)
}, [dstLocales]);

// Get locale names for end date
const getLocalesForEndDate = useCallback((end) => {
  return dstLocales.filter(loc => loc.end === end).map(loc => loc.name)
}, [dstLocales]);

const startLocales = useMemo(() => 
  getLocalesForStartDate(dstStart), [dstStart, getLocalesForStartDate]);

const endLocales = useMemo(() => 
  getLocalesForEndDate(dstEnd), [dstEnd, getLocalesForEndDate]);
...
const sleepHours = useMemo(() => { return 24 - (bedtime - wakeTime) }, 
                           [wakeTime, bedtime]);

const isDSTActive = useCallback((dayOfYear, isLeapYear) => {
  // Handle wrapping (e.g., Australia where DST spans year boundary)
  if (dstStart < dstEnd) { return dayOfYear >= dstStart && dayOfYear < dstEnd } 
  else                   { return dayOfYear >= dstStart || dayOfYear < dstEnd }
}, [dstStart, dstEnd]);
...
const calculateYearData = useCallback((isLeapYear, useDST) => {
  ...
  const summerSolsticeSunrise = 
    calculateSunrise(summerSolsticeDay, latitude, totalDays);
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    const dawnDelta = (sunrise !== null && summerSolsticeSunrise !== null) 
      ? sunrise - summerSolsticeSunrise : null;
    
    const duskDelta = (sunset !== null && summerSolsticeSunrise !== null)
      ? sunset - summerSolsticeSunrise : null;
...
const data = useMemo(() => {
  ...
  for (let day = 1; day <= 365; day++) {
    ...
    const nightAwakeAfterDusk = 
      (duskDelta !== null && adjustedWakeTime >= duskDelta && sleepTime > duskDelta && adjustedWakeTime < sleepTime) 
      ? [adjustedWakeTime, sleepTime] : null;
    // Case 2: Wake before dawn and sleep before dawn (entire wake period in morning darkness)
    const nightAwakeBeforeDawn = 
      (dawnDelta !== null && adjustedWakeTime < dawnDelta && sleepTime <= dawnDelta && adjustedWakeTime < sleepTime) 
      ? [adjustedWakeTime, sleepTime] : null;
    // Case 3: Polar night (no sunrise/sunset) - entire wake period is in darkness
    const nightAwakePolarNight = 
      (dawnDelta === null || duskDelta === null) && adjustedWakeTime < sleepTime 
      ? [adjustedWakeTime, sleepTime] : null;
...
<div className="bg-white p-6 rounded-lg shadow mb-6">
  ...
  <div className="mb-4">
    ...
    onChange={(e) => {
      ...
      setWakeTime(newWake);
      if (bedtime <= newWake) { setBedtime(newWake + 0.25) }
    }}
...
<li><a href="https://messymatters.com/dst/">Precious, Precious Daylight</a> (me blogging about this way back in 2009)</li>
</ul>
</div>
...
}; // end of DawnDeltaTool component
